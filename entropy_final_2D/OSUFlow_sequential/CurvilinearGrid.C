...
int CurvilinearGrid::get_ijk_of_cell(int cellId, VECTOR3& ijk)
{
	...
	if(cellId<0 || (cellId>=(xcelldim())*(ycelldim())*(zcelldim())))
		return -1;
	ijk[0]=cellId/((xcelldim())*(ycelldim()));
	cellId-=ijk[0]*((xcelldim())*(ycelldim()));
...
int CurvilinearGrid::get_ijk_of_vertex(int vertexId, VECTOR3& ijk)
{
	...
	if(vertexId<0 || (vertexId>=xdim()*ydim()*zdim()))
		return -1;
	ijk[0]=vertexId/(xdim()*ydim());
	vertexId-=ijk[0]*(xdim()*ydim());
...
int CurvilinearGrid::phys_to_cell(PointInfo& pInfo)
{
	...
	if(!isInBBox(pInfo.phyCoord))
		return -1;
	//at (10,10,10
	//pInfo.phyCoord=VECTOR3(0.0841770,0.0287402,0.0585942);

	Cell cell;
	//locate_initialization();
	locate(pInfo.phyCoord, cell);//pInfo.phyCoord is in cell
	//pInfo.fromCell=cell;

	int cellId=(int)cell.ijk[0]+((int)cell.ijk[1])*(xcelldim())+((int)cell.ijk[2])*(xcelldim())*(ycelldim());
	pInfo.inCell = cellId;

	//convert physical position to computational position
	VECTOR3 dp[8];

	if(1!=coordinates_at_cell(cell.ijk,dp))//return 8 velocity value
		return -1;
	...
	phys_to_comp_coords(pInfo.phyCoord, fCoeff[0], fCoeff[1], fCoeff[2]);
...
//			if ((return_iblank && (1 <= res && res <= 15)) || res == 1)
	...
	// fourth try
		...
		walk_on_z_boundary(pp, ydim()-1, min_d2, min_v);
